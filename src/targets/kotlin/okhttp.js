...
module.exports = function (source, options) {
  ...
  if (source.postData.mimeType === 'multipart/form-data' && source.postData.params && source.postData.params.length > 0) {
    hasBody = true
    code.push('val body = MultipartBody.Builder()')
    code.push(1, '.setType(MultipartBody.FORM)')
    source.postData.params.forEach(function (param) {
      if (param.fileName) {
        code.push(1, '.addFormDataPart("%s", "%s", RequestBody.create(MediaType.parse("%s"), File("%s")))', param.name, param.fileName || 'file', param.contentType || 'application/octet-stream', param.fileName || 'file')
      } else {
